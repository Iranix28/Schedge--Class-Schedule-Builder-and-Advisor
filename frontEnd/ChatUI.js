...
const BASE_URL = "http://localhost:11434/v1"; // LM Studio would be http://localhost:1234/v1
const API_KEY  = "local";                      // any string for local servers
const MODEL    = "gemma3:270m";             // match your server's model name
...
async function sendMessageLLM(userText, onToken) {
  ...
  while (true) {
    ...
    for (const evt of events) {
      ...
      // If your server emits DeepSeek reasoning: json?.choices?.[0]?.delta?.reasoning_content
    }
  }
}
...
function ChatUI() {
  ...
  const generateSampleSchedule = () => {
    return [
      { day: 'Monday', startTime: '9:00 AM', endTime: '10:30 AM', class: 'Mathematics', room: 'Room 101' },
      { day: 'Monday', startTime: '11:00 AM', endTime: '12:30 PM', class: 'Physics', room: 'Lab 203' },
      { day: 'Tuesday', startTime: '10:00 AM', endTime: '11:30 AM', class: 'Chemistry', room: 'Lab 105' },
      { day: 'Tuesday', startTime: '2:00 PM', endTime: '3:30 PM', class: 'English', room: 'Room 304' },
      { day: 'Wednesday', startTime: '9:00 AM', endTime: '10:30 AM', class: 'History', room: 'Room 201' },
      { day: 'Thursday', startTime: '1:00 PM', endTime: '2:30 PM', class: 'Biology', room: 'Lab 108' },
      { day: 'Friday', startTime: '10:00 AM', endTime: '11:30 AM', class: 'Computer Science', room: 'Lab 401' }
    ];
  };
  ...
  return (
    <div className="flex h-screen bg-slate-100 relative">
      ...
      <div className={`flex flex-col border-r border-slate-300 bg-white transition-all duration-500 ease-in-out overflow-hidden ${
            ...
            {messages.map((message, index) => (
              ...
              >
                ...
                <div
                  ...
                  style={message.role === 'user' ? { backgroundColor: '#BE0000' } : {}}
                >
        ...
        <div className="border-t border-slate-200 px-6 py-4 bg-white">
          <div>
            <div className="relative flex items-center gap-3 bg-slate-50 rounded-full border border-slate-200 px-3 py-2 focus-within:ring-2 focus-within:ring-opacity-50 transition-all"
                 style={{ focusWithinBorderColor: '#BE0000', focusWithinRingColor: '#BE0000' }}>
              <button
      ...
      <div className={`flex flex-col bg-slate-50 transition-all duration-500 ease-in-out ${
        ...
        <header className="border-b border-slate-200 px-6 py-4 h-16 flex items-center justify-between shadow-sm" style={{ backgroundColor: '#BE0000' }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center shadow-sm">
              <div className="text-xl font-bold" style={{ color: '#BE0000' }}>U</div>
            </div>
          </div>
        </header>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
              ...
              <div className="w-full">
                ...
                <div className="grid grid-cols-6 gap-1 mb-1">
                  <div className="text-xs font-semibold text-slate-600 text-center py-1">Time</div>
                  {['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map(day => (
                    <div key={day} className="text-xs font-semibold text-slate-600 text-center py-1">{day}</div>
                  ))}
                </div>
                  ...
                  <div className="space-y-0.5">
                    {['8AM', '9AM', '10AM', '11AM', '12PM', '1PM', '2PM', '3PM', '4PM', '5PM'].map(time => (
                      <div key={time} className="grid grid-cols-6 gap-1">
                        <div className="text-xs text-slate-500 py-1 text-right pr-1 h-10">{time}</div>
                        {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].map(day => (
                          <div key={`${day}-${time}`} className="h-10 border border-slate-100 rounded bg-slate-50"></div>
                        ))}
                      </div>
                    ))}
                  </div>
                          ...
                          {visualizationData.data
                            ...
                            .map((classItem, idx) => {
                              ...
                              return (
                                ...
                                >
                                  <div className="font-semibold truncate" style={{ fontSize: '10px' }}>{classItem.class}</div>
                                  <div className="truncate" style={{ fontSize: '9px', opacity: 0.9 }}>{classItem.room}</div>
                                  <div className="truncate" style={{ fontSize: '8px', opacity: 0.8 }}>{classItem.startTime}-{classItem.endTime}</div>
                                </div>
                              );
                            })}
                        </div>
      ...
      >
        {isRightPanelExpanded ? (
          <span className="text-xs font-bold" style={{ color: '#BE0000' }}>×</span>
        ) : (
          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: '#BE0000' }}></div>
        )}
      </button>
    </div>
  );
